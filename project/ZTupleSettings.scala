...
object ZTupleSettings {
  ...
  val setting3                        = scalacOptions ++= Seq("-feature", "-deprecation", "-encoding", "utf-8")
  def setting4(fmtOnCompile: Boolean) = org.scalafmt.sbt.ScalafmtPlugin.autoImport.scalafmtOnCompile := fmtOnCompile
  ...
  def commonSettings(fmtOnCompile: Boolean = true) =
    Seq(setting3, setting4(fmtOnCompile), setting5, setting7, setting10, setting11, setting12)

  val vSetting1             = version := "0.0.1-SNAP20210626"
  val vSetting2             = organization := "org.scalax.ztuple"
